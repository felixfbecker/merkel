        ...
        describe('execute()', () => {
            it('should run all its tasks', async () => {
                ...
                try {
                    ...
                    await taskList.execute(null, null, null)
                } finally {
                    stub.restore()
                }
                ...
                const taskList = TaskList.from([
                    new Task({
                        ...
                        migration: new Migration({
                            name: 'a4bdf4cc-f7ed-4785-8bdb-2abfb5ed2025',
                        }),
                    }),
                    new Task({
                        type: 'up',
                        migration: new Migration({
                            name: 'e55d537d-ad67-40e6-9ef1-50320e530676',
                        }),
                    }),
                ])
                assert.equal(
            ...
            it('should wrap long commands', async () => {
                const taskList = TaskList.from([
                    ...
                    new Task({
                        type: 'up',
                        migration: new Migration({
                            name: '15ca72e9-241c-4fcd-97ad-aeb367403a27',
                        }),
                    }),
                    new Task({
                        type: 'up',
    ...
    describe('Task', () => {
        ...
        describe('execute()', () => {
            ...
            it('should execute an up task', async () => {
                const task = new Task({
                    type: 'up',
                    migration: new Migration({ name: 'test_migration' }),
                })
            ...
            it('should throw a MigrationNotFoundError if the migration is not existent', async () => {
                const task = new Task({
                    type: 'up',
                    migration: new Migration({ name: 'not_found' }),
                })
                const head = new Commit({ sha1: 'HEADCOMMITSHA1' })
                const trigger = new Commit({ sha1: 'TRIGGERCOMMITSHA1' })
            ...
            it('should throw a MigrationLoadError if the migration fails loading', async () => {
                const task = new Task({
                    type: 'up',
                    migration: new Migration({ name: 'error_load' }),
                })
                const head = new Commit({ sha1: 'HEADCOMMITSHA1' })
                const trigger = new Commit({ sha1: 'TRIGGERCOMMITSHA1' })
                await assert.isRejected(
            ...
            it('should throw a TaskTypeNotFoundError if the migration has no up or down function', async () => {
                const task = new Task({
                    type: 'up',
                    migration: new Migration({ name: 'no_up' }),
                })
                const head = new Commit({ sha1: 'HEADCOMMITSHA1' })
                const trigger = new Commit({ sha1: 'TRIGGERCOMMITSHA1' })
                await assert.isRejected(
            ...
            it('should throw a MigrationExecutionError when the migration returns a rejected promise', async () => {
                const task = new Task({
                    type: 'up',
                    migration: new Migration({ name: 'error_async' }),
                })
                const head = new Commit({ sha1: 'HEADCOMMITSHA1' })
                const trigger = new Commit({ sha1: 'TRIGGERCOMMITSHA1' })
                await assert.isRejected(
            ...
            it('should throw a MigrationExecutionError when the migration throws sync', async () => {
                const task = new Task({
                    type: 'up',
                    migration: new Migration({ name: 'error_sync' }),
                })
                const head = new Commit({ sha1: 'HEADCOMMITSHA1' })
                const trigger = new Commit({ sha1: 'TRIGGERCOMMITSHA1' })
                await assert.isRejected(
            ...
            it('should throw a MigrationExecutionError when the migration would crash the process', async () => {
                const task = new Task({
                    type: 'up',
                    migration: new Migration({ name: 'error_crash' }),
                })
                const head = new Commit({ sha1: 'HEADCOMMITSHA1' })
                const trigger = new Commit({ sha1: 'TRIGGERCOMMITSHA1' })
        ...
        describe('toString', () => {
            it('should throw if the task type is unknown', async () => {
                const task = new Task({ type: 'd' } as any)
